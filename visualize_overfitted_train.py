...
def plot_samples_grid(alpha_vals, beta_vals, loss_grids, metas, save_path,
    ...
    plt.suptitle('CIFAR-100 Overfitted Model - Train Set Loss Landscapes (Filter-Normalized)',
...
def main():
    ...
    NUM_SAMPLES = 10
    SAMPLE_SET = 'train'  # Train set for this version
    RESOLUTION = 51
    ...
    print("=" * 60)
    print("CIFAR-100 Overfitted Model - Train Set Single-Sample Landscapes")
    print("=" * 60)
    ...
    print("\n1. Loading overfitted model...")
    model = TinyCNN().to(DEVICE)
    checkpoint = torch.load('cifar100_cnn_overfitted.pth', map_location=DEVICE)
    model.load_state_dict(checkpoint['model_state_dict'])
    model.eval()
    print(f"   Overfitted model loaded. Test accuracy: {checkpoint['test_accuracy']:.2f}%")
    print(f"   Fail dataset accuracy: {checkpoint['fail_dataset_accuracy']:.2f}%")
    ...
    with open('loss_landscape_results/selected_samples_overfitted_train.json', 'w') as f:
        json.dump({
            'seed': SEED,
    ...
    print("   Saved: loss_landscape_results/selected_samples_overfitted_train.json")
    
    print("\n8. Creating visualization...")
    plot_samples_grid(
        alpha_vals, beta_vals, loss_grids, metas,
        'loss_landscape_results/samples_landscape_overfitted_train.png',
        layout=LAYOUT,
        share_colorbar=True,
    ...
    plot_samples_grid(
        alpha_vals, beta_vals, loss_grids, metas,
        'loss_landscape_results/samples_landscape_overfitted_train.pdf',
        layout=LAYOUT,
        share_colorbar=True,
        color_scale=COLOR_SCALE
    )
    ...
    for i, (idx, loss_grid) in enumerate(zip(selected_indices, loss_grids)):
        np.savez(f'loss_landscape_results/sample_{idx}_overfitted_train.npz',
                 alpha=alpha_vals,
                 beta=beta_vals,
                 loss=loss_grid,
                 idx=idx,
    ...
    with open('loss_landscape_results/overfitted_train_experiment_log.txt', 'w') as f:
        f.write("CIFAR-100 Overfitted Model - Train Set Landscapes\n")
        f.write("=" * 60 + "\n\n")
